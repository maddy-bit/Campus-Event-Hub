...
/* ── Config ─────────────────────────────────────────── */
const TYPE_CONFIG = {
  Announcement: { icon: Megaphone, color: "bg-[#B6FF60]", border: "border-[#8bc34a]", accent: "#B6FF60" },
  Reminder:     { icon: Clock, color: "bg-[#c2d9ff]", border: "border-[#5b9bf5]", accent: "#c2d9ff" },
  Alert:        { icon: AlertTriangle, color: "bg-[#ff6b6b]", border: "border-[#e53935]", accent: "#ff6b6b" },
  Update:       { icon: RefreshCw, color: "bg-[#fff35c]", border: "border-[#f9a825]", accent: "#fff35c" },
};
...
const Notification = () => {
  ...
  const handleMarkRead = async (id) => {
    try {
      ...
      toast.success("Marked as read — will auto-clear in 24h");
    } catch (err) {
  ...
  const handleMarkAllRead = async () => {
    try {
      ...
      toast.success("All marked as read — will auto-clear in 24h");
    } catch (err) {
  ...
  const filters = ["All", "Unread", "Announcement", "Alert", "Update", "Reminder"];
    ...
    <div className="min-h-screen font-mono text-black uppercase">
      <div className="max-w-3xl mx-auto">
        ...
        <header className="mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-5">
            <div>
              <p className="text-[10px] font-bold text-gray-400 mb-1 flex items-center gap-1.5">
                <Sparkles size={10} /> STUDENT // NOTIFICATIONS
              </p>
          ...
          {/* Auto-clear info banner */}
          <div className="bg-[#fff35c] border-3 border-black px-4 py-2.5 mb-5 shadow-[3px_3px_0px_#000] flex items-center gap-3">
            <Archive size={14} className="shrink-0" />
            <p className="text-[10px] font-bold normal-case">
              Read notifications are automatically cleared after 24 hours to keep your inbox clean.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="border-4 border-dashed border-black bg-white p-16 text-center shadow-[4px_4px_0px_rgba(0,0,0,0.05)]">
            ...
            <p className="text-base font-black text-gray-500 mb-1">
              {filter === "All" ? "NO NOTIFICATIONS YET" : `NO ${filter.toUpperCase()} NOTIFICATIONS`}
            </p>
            <p className="text-[11px] text-gray-400 normal-case font-bold">
              {filter === "All"
                ? "Register for events to start receiving notifications."
