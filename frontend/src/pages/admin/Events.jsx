        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
            ...
            <button type="submit" disabled={loading} className="px-8 py-3 rounded-xl font-bold bg-black text-white hover:bg-gray-900 shadow-xl shadow-black/10 transition-all disabled:opacity-70 flex items-center justify-center min-w-[140px]">
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : initialData ? "Save Changes" : "Create Event"}
            </button>
          </div>
        </form>
      </div>
    ...
    <div className="pb-24 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
        ...
        <button onClick={() => { setEditingEvent(null); setShowModal(true); }}
          className="w-full md:w-auto flex items-center justify-center gap-2 bg-black text-white px-6 py-3 rounded-2xl font-bold transition-all hover:-translate-y-0.5 hover:shadow-xl active:translate-y-0">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Search events..." value={search} onChange={e => setSearch(e.target.value)}
              className="w-full sm:w-56 pl-10 pr-3 py-2.5 bg-white border border-slate-200 rounded-2xl text-sm font-medium focus:outline-none focus:border-black focus:ring-1 focus:ring-black shadow-sm" />
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-dashed border-slate-300 flex flex-col items-center justify-center py-20 px-4 text-center">
          ...
          <button onClick={() => { setEditingEvent(null); setShowModal(true); }}
            className="text-black font-bold flex items-center gap-2 hover:bg-slate-100 px-5 py-2.5 rounded-xl transition-colors text-sm">
      ...
      <EventModal show={showModal} onClose={() => { setShowModal(false); setEditingEvent(null); }}
